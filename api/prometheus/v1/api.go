...
// # Architecture
//
// The public API is implemented by httpAPI, which relies on the internal
// apiClient interface for all HTTP communication. The production implementation
// of that interface is apiClientImpl, which delegates to three internal
// sub-packages:
//
//   - internal/codec  – JSON registration and response-envelope parsing
//   - internal/transport – POST-with-GET-fallback HTTP execution
//   - internal/endpoints – endpoint path constants
//
// This separation keeps serialization, wire-protocol, and routing concerns
// distinct while leaving all exported types in the public package so callers
// see stable import paths.
package v1
...
import (
	...
	// Import codec for its init() side effect: registering the custom
	// json-iterator encoders/decoders for Prometheus model types.
...
// apiResponse is the envelope returned by the Prometheus HTTP API.
// It is retained in the public package so that test helpers in the same
// package can construct mock responses without importing internal/codec.
...
// parseHTTPResponse interprets a raw HTTP response and body from the Prometheus
// API and returns the data payload, warnings, and any error. It is shared by
// both Do and DoGetFallback to avoid duplicating the response-parsing logic.
func (h *apiClientImpl) parseHTTPResponse(resp *http.Response, body []byte) (*http.Response, []byte, Warnings, error) {
	...
	result, jsonErr := codec.ParseResponse(body)
	if jsonErr != nil {
		return resp, body, nil, &Error{
			Type: ErrBadResponse,
			Msg:  jsonErr.Error(),
...
func (h *apiClientImpl) Do(ctx context.Context, req *http.Request) (*http.Response, []byte, Warnings, error) {
	...
	return h.parseHTTPResponse(resp, body)
}
...
func (h *apiClientImpl) DoGetFallback(ctx context.Context, u *url.URL, args url.Values) (*http.Response, []byte, Warnings, error) {
	resp, body, err := transport.DoGetFallback(ctx, h.client, u, args)
	if err != nil {
		return resp, body, nil, err
	}
	return h.parseHTTPResponse(resp, body)
