...
const (
	epAlerts          = endpoints.Alerts
	epAlertManagers   = endpoints.AlertManagers
	epQuery           = endpoints.Query
	epQueryRange      = endpoints.QueryRange
	epQueryExemplars  = endpoints.QueryExemplars
	epLabels          = endpoints.Labels
	epLabelValues     = endpoints.LabelValues
	epSeries          = endpoints.Series
	epTargets         = endpoints.Targets
	epTargetsMetadata = endpoints.TargetsMetadata
	epMetadata        = endpoints.Metadata
	epRules           = endpoints.Rules
	epSnapshot        = endpoints.Snapshot
	epDeleteSeries    = endpoints.DeleteSeries
	epCleanTombstones = endpoints.CleanTombstones
	epConfig          = endpoints.Config
	epFlags           = endpoints.Flags
	epBuildinfo       = endpoints.Buildinfo
	epRuntimeinfo     = endpoints.Runtimeinfo
	epTSDB            = endpoints.TSDB
	epTSDBBlocks      = endpoints.TSDBBlocks
	epWalReplay       = endpoints.WalReplay
	epFormatQuery     = endpoints.FormatQuery
)
...
func (h *apiClientImpl) Do(ctx context.Context, req *http.Request) (*http.Response, []byte, Warnings, error) {
	resp, body, warnings, err := internaltransport.Do(ctx, h.client, req, transportConfig())
...
func (h *apiClientImpl) DoGetFallback(ctx context.Context, u *url.URL, args url.Values) (*http.Response, []byte, Warnings, error) {
	resp, body, warnings, err := internaltransport.DoGetFallback(ctx, h.client, u, args, transportConfig())
	return resp, body, Warnings(warnings), err
}

func transportConfig() internaltransport.Config {
	return internaltransport.Config{
		Unmarshal: codec.Unmarshal,
		APIError:  apiError,
		StatusError: func(resp *http.Response) (string, string) {
			errType, msg := errorTypeAndMsgFor(resp)
			return string(errType), msg
		},
		NewError: func(errType, msg, detail string) error {
			return &Error{
