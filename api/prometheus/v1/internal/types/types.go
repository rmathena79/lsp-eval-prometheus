...
// Package types contains all data types, constants, and option machinery for
// the Prometheus v1 HTTP API client. It is intentionally free of HTTP and
// serialisation logic so that it can be imported by every other internal
// subpackage without creating import cycles.
package types
...
const AllStatsValue StatsValue = "all"

// APIOptions holds per-request option values applied by Option functions.
...
// UnmarshalJSON implements json.Unmarshaler for RuleGroup. It decodes each
// rule element into either an AlertingRule or a RecordingRule based on the
// "type" field.
...
// UnmarshalJSON implements json.Unmarshaler for AlertingRule. It validates
// that the rule "type" field equals "alerting".
...
// UnmarshalJSON implements json.Unmarshaler for RecordingRule. It validates
// that the rule "type" field equals "recording".
