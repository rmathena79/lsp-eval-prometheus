...
func UnmarshalQueryResultValue(b []byte) (model.Value, error) {
	var raw internaltypes.QueryResult
	...
	case model.ValScalar:
		var sv model.Scalar
		if err := json.Unmarshal(raw.Result, &sv); err != nil {
			return nil, err
		}
		return &sv, nil
	case model.ValVector:
		var vv model.Vector
		if err := json.Unmarshal(raw.Result, &vv); err != nil {
			return nil, err
		}
		return vv, nil
	case model.ValMatrix:
		var mv model.Matrix
		if err := json.Unmarshal(raw.Result, &mv); err != nil {
			return nil, err
		}
		return mv, nil
	default:
		return nil, fmt.Errorf("unexpected value type %q", raw.Type)
	}
}
...
func marshalFloat(v float64, stream *json.Stream) {
	...
	buf = strconv.AppendFloat(buf, v, fmtChar, -1, 64)
